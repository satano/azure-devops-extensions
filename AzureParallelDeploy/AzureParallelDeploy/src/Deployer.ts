...
export class Deployer {
	...
	public async deployWebApps(services: string[]): Promise<boolean> {
		...
		console.log(
			tl.loc("DeployingServicesBaseFolder", this.settings.appSourceBasePath)
		);
		console.log("");
		...
		services.forEach((service) => {
			...
			var targetService = Utility.formatString(
				this.settings.appNameFormat,
				service
			);
			var sourceFileName = Utility.formatString(
				this.settings.appSourceFormat,
				service
			);
			...
			var sourceFiles = tl.findMatch(
				this.settings.appSourceBasePath,
				`**/${sourceFileName}`
			);
			if (sourceFiles.length == 0) {
				result = false;
				Utility.logError(
					tl.loc("DeployingServiceNoSourceFile", sourceFileName, service)
				);
				return;
			} else if (sourceFiles.length > 1) {
				result = false;
				Utility.logError(
					tl.loc("DeployingServiceMoreSourceFiles", sourceFileName, service)
				);
			...
			var command =
				this.settings.appType == AppType.FunctionApp ? "functionapp" : "webapp";
			var slotNameParam = Utility.isNullOrWhitespace(this.settings.slotName)
				? ""
				: ` --slot ${this.settings.slotName}`;
			var azArgs =
				`${command} deployment source config-zip` +
		...
		return tl.exec("az", azArgs).then(
			...
			(error) => {
				...
				if (this.retryCount[service] < Deployer.maxRetries) {
					...
					console.log(
						`${service}: ${tl.loc("DeployServiceRetry", retryCount)}`
					);
